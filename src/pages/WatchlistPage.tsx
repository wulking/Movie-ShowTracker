...
export default function WatchlistPage({
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getMediaByStatus("watchlist");
      setItems(data);
    } catch (err) {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {displayed.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-accent/20 text-accent-light hover:bg-accent/30 border border-accent/30 transition-all"
            >
              <Shuffle size={16} />
              Surprise Me
            </button>
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            <Bookmark size={48} className="mx-auto text-fr-border mb-4" />
            ...
            <p className="text-fr-subtle text-sm mt-1">
              {items.length === 0
                ? "Search for movies and shows to add to your watchlist"
